...
# Where to find data
tf.app.flags.DEFINE_string('data_path', r'D:\datasets\yelp',
                           'Path expression to tf.Example datafiles. '
                           'Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('vocab_path', r'D:\datasets\yelp\vocab.txt',
                           'Path expression to text vocabulary file.')

# Important settings
tf.app.flags.DEFINE_string('mode', 'train_generator',
...
tf.app.flags.DEFINE_string('exp_name', 'experiment',
...
tf.app.flags.DEFINE_float('rand_unif_init_mag', 0.02,
                          'magnitude for lstm cells random uniform initialization')  # for discriminator and generator
tf.app.flags.DEFINE_float('trunc_norm_init_std', 1e-4,  # for discriminator and generator
...
'''
the generator model is saved at FLAGS.log_root + "train-generator give up sv, use sess
'''
...
def main(unused_argv):
    ...
    if hps_generator.mode == 'train_generator':
    ...
    elif hps_generator.mode == 'adversarial_train':
        print("Start pre-training......")
        model = Generator(hps_generator, vocab)

        sess_ge, saver_ge, train_dir_ge = utils.setup_model(model)
        generated = Generated_sample(model, vocab, batcher, sess_ge)
        ...
        dis_batcher = DisBatcher(hps_discriminator, vocab, "discriminator_train/positive/*",
                                 ...
                                 "discriminator_test/negative/*")
        sess_dis, saver_dis, train_dir_dis = setup_training_discriminator(model_dis)
        utils.load_ckpt(saver_dis, sess_dis, ckpt_dir="train-discriminator")
        utils.load_ckpt(saver_ge, sess_ge, ckpt_dir="train-generator")
        ...
        generated.generator_test_sample_example(
            "MLE/" + "MLE_sample_positive", "MLE/" + "MLE_sample_negative", 200)

        print("evaluate the diversity of MLE (decode based on max probability)")
        generated.generator_test_max_example(
            "MLE/" + "MLE_max_temp_positive", "MLE/" + "MLE_max_temp_negative", 200)
        ...
        for epoch in range(10):
            ...
            for step in range(int(len(batches) / 1000)):
                run_train_generator(model, model_dis, sess_dis, batcher, dis_batcher,
                                    batches[step * 1000:(step + 1) * 1000], sess_ge, saver_ge, train_dir_ge, generated)
                generated.generator_sample_example(
                    "train_sample_generated/" + str(epoch) + "epoch_step" + str(step) + "_temp_positive",
                    "train_sample_generated/" + str(epoch) + "epoch_step" + str(step) + "_temp_negative", 1000)
                ...
                for i in range(epoch + 1):
                    for j in range(step + 1):
                        dis_batcher.train_queue += dis_batcher.fill_example_queue(
                            "train_sample_generated/" + str(i) + "epoch_step" + str(j) + "_temp_positive/*")
                        dis_batcher.train_queue += dis_batcher.fill_example_queue(
                            "train_sample_generated/" + str(i) + "epoch_step" + str(j) + "_temp_negative/*")
...
def output_to_batch(current_batch, result, batcher, dis_batcher):
    ...
    for i in range(FLAGS.batch_size):
        ...
        if decoded_words_all.strip() == "":
            ...
            new_dis_example = bd.Example(current_batch.original_review_output[i], -0.0001, dis_batcher._vocab,
                                         dis_batcher._hps)
    ...
    return Batch(example_list, batcher._hps, batcher._vocab), bd.Batch(db_example_list, dis_batcher._hps,
                                                                       dis_batcher._vocab)


def run_train_generator(model, discirminator_model, discriminator_sess, batcher, dis_batcher, batches, sess, saver,
                        train_dir, generated):
    ...
    while step < len(batches):
        ...
        for i in range(FLAGS.batch_size):
            for j in range(batcher._hps.max_enc_sen_num):
                ...
                a = {"example": current_batch.review_sentenc_orig[i][j],
