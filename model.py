...
class Generator(object):
    ...
    def _add_placeholders(self):
        ...
        self._dec_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_sen_num, hps.max_dec_steps],
                                         name='dec_batch')
        self._target_batch = tf.placeholder(tf.int32, [hps.batch_size * hps.max_dec_sen_num, hps.max_dec_steps],
                                            name='target_batch')
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if FLAGS.run_method == 'auto-encoder':
                    ...
                    sentence_level_input = tf.reshape(tf.tile(tf.expand_dims(self._dec_in_state.h, axis=1),
                                                              [1, hps.max_dec_steps, 1]),
                                                      [hps.batch_size, hps.max_dec_sen_num, hps.hidden_dim])
                    tf.logging.info(encoder_outputs_word)  # reduced
                    encoder_outputs_word = tf.reshape(tf.tile(tf.expand_dims(encoder_outputs_word, axis=1),
...
def sample_output(embedding, embedding_dec, output_projection=None, given_number=None):
