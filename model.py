...
def sample_output(embedding, output_projection=None):
...
class Generator(object):
    ...
    def _add_placeholders(self):
        ...
        self._target_batch = tf.placeholder(
            tf.int32, [hps.batch_size * hps.max_dec_sen_num, hps.max_dec_steps], name='target_batch')

        self._dec_padding_mask = tf.placeholder(
            tf.float32, [hps.batch_size * hps.max_dec_sen_num, hps.max_dec_steps], name='dec_padding_mask')
    ...
    def _add_encoder(self, encoder_inputs, seq_len):  # bidirectional rnn
    ...
    def _add_decoder(self, loop_function, loop_function_max, loop_given_function, input, attention_state):
        ...
        return [decoder_outputs_pretrain, decoder_outputs_sample_generator,
                decoder_outputs_max_generator, decoder_outputs_given_sample_generator]
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if FLAGS.run_method == 'auto-encoder':
                    ...
                    sentence_level_input = tf.reshape(
                        tf.tile(tf.expand_dims(self._dec_in_state.h, axis=1), [1, hps.max_dec_sen_num, 1]),
                        [hps.batch_size, hps.max_dec_sen_num, hps.hidden_dim])
                    tf.logging.info(encoder_outputs_word)  # reduced
                    ...
                    (encoder_outputs, _) = tf.nn.dynamic_rnn(
                        sentence_level_cell, sentence_level_input,
                        dtype=tf.float32, sequence_length=self.dec_lens, swap_memory=True)
            ...
            [decoder_outputs_pretrain, decoder_outputs_sample_generator,
             decoder_outputs_max_generator, decoder_outputs_given_sample_generator] = self._add_decoder(
                loop_function=loop_function, loop_function_max=loop_function_max,
                loop_given_function=loop_given_function, input=emb_dec_inputs, attention_state=encoder_outputs_word)

            decoder_outputs_pretrain = tf.reshape(
                decoder_outputs_pretrain,
                [hps.batch_size * hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
            decoder_outputs_pretrain = tf.reshape(
                tf.nn.xw_plus_b(decoder_outputs_pretrain, w, v),
                [hps.batch_size * hps.max_dec_sen_num, hps.max_dec_steps, vsize])

            decoder_outputs_sample_generator = tf.reshape(
                decoder_outputs_sample_generator,
                [hps.batch_size * hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
            self._sample_best_output = tf.reshape(
                tf.argmax(tf.nn.xw_plus_b(decoder_outputs_sample_generator, w, v), 1),
                [hps.batch_size, hps.max_dec_sen_num, hps.max_dec_steps])

            decoder_outputs_given_sample_generator = tf.reshape(
                decoder_outputs_given_sample_generator,
                [hps.batch_size * hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
            self._sample_given_best_output = tf.reshape(
                tf.argmax(tf.nn.xw_plus_b(decoder_outputs_given_sample_generator, w, v), 1),
                [hps.batch_size, hps.max_dec_sen_num, hps.max_dec_steps])

            decoder_outputs_max_generator = tf.reshape(
                decoder_outputs_max_generator,
                [hps.batch_size * hps.max_dec_sen_num * hps.max_dec_steps, hps.hidden_dim])
            ...
            loss = tf.contrib.seq2seq.sequence_loss(
                decoder_outputs_pretrain,  # batch_size * dec_sen_num, dec_max_step, vocab_size
                self._target_batch,
    ...
    def run_pre_train_step(self, sess, batch):
        ...
        to_return = {'train_op': self._train_op, 'loss': self._cost, 'global_step': self.global_step, }
        return sess.run(to_return, feed_dict)

    def run_eval_given_step(self, sess, batch):
        feed_dict = self._make_feed_dict(batch)
        to_return = {'generated': self._sample_given_best_output, }
        return sess.run(to_return, feed_dict)
